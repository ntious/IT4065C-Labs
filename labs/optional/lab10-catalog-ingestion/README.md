...
Complete [Lab 2](../../core/lab02-classification/README.md) and use the same configured Ubuntu environment. Run commands from the repository root. A retailer provides CSV transactions, JSON events and a
...
```bash
.venv/bin/python labs/optional/lab10-catalog-ingestion/catalog_contract_example.py
```
...
```bash
cp -i labs/optional/lab10-catalog-ingestion/catalog_contract_example.py .local/my-ticket-contract.py
```
