...
We recommend completing [Lab 1](../lab01-environment/README.md),
[Lab 2](../lab02-classification/README.md) and
[Lab 3](../lab03-modeling-quality/README.md) first. They introduce the environment,
...
LAB 4 TECHNICAL CHECKS COMPLETE. Return to labs/core/lab04-lineage-lifecycle/README.md for interpretation and deliverables.
```

![Annotated Lab 4 output: connection and seed checks, model and test counts, documentation and lineage checks, and the remaining investigation.](../../../sample_screenshots/lab4-technical-output-guide.svg)

*Annotated reference for interpreting output, not an execution screenshot.
Use the command above for copying; the table below explains each result.*
...
<details>
<summary>Optional reference: the order paths explained</summary>
...
`order_detail_mart`. The annotated paths below show where to look; inspect the
same connections in your own graph.

![Order lineage: raw.orders leads to stg_orders, then fct_orders, which feeds both order_detail_mart and olap_sales_by_day. The two named Lab 3 test nodes are labeled separately from reporting models.](../../../sample_screenshots/lab4-order-lineage-guide.svg)

*Annotated dependency reference, not evidence of a new execution.
The text paths below provide the same guidance.*
...
```bash
cp -i labs/core/lab04-lineage-lifecycle/_turnin_template.md .local/lab4-decision-log.md
```
