...
**Where it shows up**
- Step 8/9 expects dbt to connect as `user: postgres` with password `"REMOVED_HISTORICAL_PASSWORD"`.  
...
```bash
sudo -u postgres psql -c "ALTER USER postgres PASSWORD 'REMOVED_HISTORICAL_PASSWORD';"
```
...
```
Use password: `REMOVED_HISTORICAL_PASSWORD` fileciteturn0file0
...
it4065c_platform:
  ...
  outputs:
    dev:
      type: postgres
      user: postgres
      password: "REMOVED_HISTORICAL_PASSWORD"
      host: localhost
      port: 5432
      dbname: it4065c
      schema: student_kofi
